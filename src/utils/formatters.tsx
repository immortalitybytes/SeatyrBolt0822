...
/**
 * Format a guest name for display, rendering the % symbol and word after it in dark grey.
 */
export const formatGuestNameWithPercent = (name: string): JSX.Element => {
  if (!name.includes('%')) return <>{name}</>;
  
  const [beforePercent, afterPercent] = name.split('%');
  
  return (
    <>
      {beforePercent}
      <span style={{ color: '#959595' }}>%{afterPercent}</span>
    </>
  );
};